...
for mass in [110,115,120,125,130,135,140,145] :
  ...
  for i1 in toPlot :
    ...
    for i in dirList :
      ...
      ztt = copy.deepcopy(gDirectory.Get('../svfitMass*1.03/ZTT'))

      for m in [ztt,qcd,ewk,hig,ttb] :
	    m.UseCurrentStyle()
	    #m.Scale(0.83)   ####### dangerous, for SS only
	    m.SetLineWidth(2)
	    m.Rebin(rebin)
	    m.SetTitle('')
...
#       for bin in range(qcd.GetNbinsX()) :
#         x = bin*qcd.GetBinWidth(bin)
#         #scale = -0.066*x + 1.16    #con Jet30
#         scale = -0.159*x + 1.39     #con Jet50
#         qcd.SetBinContent(bin, scale*qcd.GetBinContent(bin) )
      ...
      c2 = TCanvas("canvas_"+str(i.GetName())+str(mass),"canvas_"+str(i.GetName()),700,770)
      ...
      #for s1 in [qcd,ewk,ttb,ztt,hig] : # order matters
      for s1 in [qcd,ewk,ttb,ztt] : #,hig] : # order matters
        stack.Add(s1)
